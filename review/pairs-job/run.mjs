...
function judgePrompt(count) {
  return `You are judging ${count} photographs for realism and fit. They are attached in order, numbered 1 to ${count}. For each, reply with JSON only: an array of {"n", "realism": 1-5 (5 = indistinguishable from a real camera photo), "waxy": true if any skin looks smoothed, plastic or waxy, "aiTells": short list of visible AI artefacts or [], "gritty": true if it reads as poverty, decay, litter, graffiti or squalor, "subjectTop": % of the image height where the main subject starts, "subjectBottom": % where it ends, "calmBottom": true if the bottom third is calm and empty}. Judge only what you can see. Do not run commands or read files.`;
}
// Blind judge of every take, then the pick: realism first (no waxy skin, no grit), then how well it is composed.
// A pair is pre-marked USE only when its pick is realistic (4+), not waxy and not gritty. Home D is not live:
// its joke band (about 55-75 % down the screen) is reported on the pair, not a reason to say NO.
function judgeAndChoose(pairs, cwd, outName) {
  const all = pairs.flatMap((l) => (l.takes || []).map((t, i) => ({ l, t, i })));
  if (all.length) {
    const jr = codex(judgePrompt(all.length), { images: all.map((a) => a.t), out: path.join(cwd, outName), cwd });
    let marks = [];
  ...
  for (const l of pairs) {
    ...
    const score = (a) => (a.m ? a.m.realism * 10 - (a.m.waxy ? 20 : 0) - (a.m.gritty ? 20 : 0) - (a.m.aiTells?.length || 0) * 3 + (a.m.calmBottom ? 2 : 0) - (a.m.subjectBottom > 55 ? 5 : 0) : 0);
    ...
    l.realOk = !!(best?.m && !best.m.waxy && !best.m.gritty && best.m.realism >= 4);
...
async function rebuildPage(pn) {
  ...
  const kept = b.pairs.map((p) => {
    ...
    return { ...p, realOk: p.realOk ?? !!(best && !best.waxy && !best.gritty && best.realism >= 4) };
  });
...
if (process.argv.includes('--rejudge')) {
  ...
  judgeAndChoose(b.pairs, bdir, 'judge-again.txt');
...
Nothing written anywhere: no text, letters, signs, logos, labels or number plates.`;
  ...
  if (l.takes.length > 2) { log(`${l.id}: Sol made ${l.takes.length} takes; keeping the first 2 (cap)`); l.takes = l.takes.slice(0, 2); }
  images += l.takes.length;
  log(`${l.id}: ${l.takes.length} takes`);
}

// ---- 8: blind judge + the covers-the-subject check ------------------------------------------------
judgeAndChoose(kept, dir, 'judge.txt');
