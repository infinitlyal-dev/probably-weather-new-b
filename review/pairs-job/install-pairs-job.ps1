...
#
# Copies run.mjs, RECIPE.md and opus-lines.json to %USERPROFILE%\pw-pairs-job\ (so it runs whatever
...
foreach ($f in 'run.mjs', 'RECIPE.md', 'opus-lines.json') { Copy-Item -Force (Join-Path $PSScriptRoot $f) (Join-Path $RuntimeDir $f) }
