...
// imports remain split: install UI, the bespoke line tables (hero-lines.js and
// its Afrikaans, hero-lines-af.js) and one chunk for each language bank.
...
const lazyEntryPoints = Object.values(clientBundle.metafile.outputs)
  ...
  .filter(Boolean);
for (const required of ['install.js', 'hero-lines.js', 'hero-lines-af.js', 'copy/en.js', 'copy/af.js', 'copy/zu.js', 'copy/xh.js', 'copy/st.js']) {
