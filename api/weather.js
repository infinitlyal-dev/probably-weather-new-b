...
export default async function handler(req, res) {
  try {
    ...
    const latQ = req.query.lat !== undefined ? Number(req.query.lat) : null;
    const lonQ = req.query.lon !== undefined ? Number(req.query.lon) : null;
    const hasCoords = isNum(latQ) && isNum(lonQ);

    if (!hasCoords && !q) {
      return res.status(400).json({ error: "Missing query param: q OR lat/lon" });
    }
    ...
    const normalizeCondition = ({ code, text }) => {
      ...
      if (t.includes("cloud") || t.includes("overcast") || t.includes("fog") || t.includes("mist"))
    ...
    // IMPORTANT: for coords mode, query by "lat,lon" so small towns never fail.
    const waQuery = hasCoords ? `${lat},${lon}` : name;
